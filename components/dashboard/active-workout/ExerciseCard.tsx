...
export const ExerciseCard = ({ 
  ...
  const similarExercises = useMemo(() => {
    return exerciseStore.exercises.filter((ex: Exercise) => 
      ex.id !== exercise.id && (
        ex.primaryMuscles.some((muscle: string) => 
          exercise.primaryMuscles.includes(muscle) || 
          exercise.secondaryMuscles.includes(muscle)
        ) ||
        ex.secondaryMuscles.some((muscle: string) => 
          exercise.primaryMuscles.includes(muscle)
        )
      )
    ).slice(0, 5);
            ...
            <View className="flex-row justify-between items-center">
              ...
              <View className="flex-row items-center gap-2">
                <View className="items-end mr-2">
                  <View className={`flex-row items-center gap-1 px-2 py-1 rounded-lg ${isActive ? 'bg-primary/10' : 'bg-secondary/10'}`}>
              ...
              <View className="mt-3">
                ...
                <View className="h-2 bg-secondary/10 rounded-full overflow-hidden">
                  <View 
                    ...
                    style={{ width: `${(workoutExercise.completedSets.filter(set => set.isCompleted).length / workoutExercise.sets) * 100}%` }} 
                  />
                </View>
              </View>
            )}
                        ...
                        <View className="ml-3 flex-1">
                          ...
                          <View className="flex-row flex-wrap gap-1 mt-1">
                            {exercise.primaryMuscles.map((muscle, index) => (
                              <View key={`muscle-${exercise.id}-${muscle}-${index}`} className="bg-primary/10 rounded-full px-2 py-0.5">
