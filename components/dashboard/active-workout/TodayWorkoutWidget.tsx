//components\dashboard\active-workout\TodayWorkoutWidget.tsx
import React from 'react';
import { View, Pressable, Image } from 'react-native';
import { Text } from '~/components/ui/text';
import { Card } from '~/components/ui/card';
import { Dumbbell, Clock, CalendarCheck, ChevronRight, Flame, Zap, Share2, Play } from 'lucide-react-native';
...
export const TodayWorkoutWidget = ({ workout }: TodayWorkoutWidgetProps) => {
  ...
  const previewExercises = workout.exercises
    .slice(0, isExpanded ? workout.exercises.length : 3)
    .map(workoutExercise => {
      return exerciseStore.exercises.find(ex => ex.id === workoutExercise.exerciseId);
    })
    .filter(Boolean);

  const remainingExercises = workout.exercises.length - (isExpanded ? workout.exercises.length : 3);
  const estimatedCalories = workout.exercises.length * 50; // Simple estimation
  ...
  const handlePress = () => {
    ...
    // @ts-ignore
    router.push(`/dashboard/active-workout/${workout.id}`);
  };
  ...
  return (
    ...
    >
      <Card className="overflow-hidden bg-card/60 backdrop-blur-lg border border-white/10">
        {/* Progress Bar */}
        <View className="w-full h-1 bg-muted">
          <View 
...
<Pressable onPress={handleExpand}>
  <View className="my-4">
    {workout.exercises.slice(0, isExpanded ? workout.exercises.length : 3).map((workoutExercise, index) => {
      const exercise = exerciseStore.exercises.find(ex => ex.id === workoutExercise.exerciseId);
