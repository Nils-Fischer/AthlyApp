...
const SelectItem = React.forwardRef<SelectPrimitive.ItemRef, SelectPrimitive.ItemProps>(
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        "relative web:group flex flex-row w-full web:cursor-default web:select-none items-center rounded-sm py-1.5 native:py-2 pl-8 native:pl-10 pr-2 web:hover:bg-muted active:bg-muted web:outline-none web:focus:bg-muted",
    ...
    >
      ...
      <SelectPrimitive.ItemText className="text-sm native:text-lg text-popover-foreground web:group-focus:text-muted-foreground" />
    </SelectPrimitive.Item>
  )
);
